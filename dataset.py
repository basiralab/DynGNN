...
def multivariate_simulate(n_samples=200,n_time=2,n_views=1):
...
def prepare_data(new_data=False, n_samples=200, n_times=6, name=''):
    ...
    try:
        if new_data:
            ...
            np.save(f'./multivariate_simulation_data_{name}.npy',samples)
        else:
